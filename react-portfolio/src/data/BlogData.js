export const Blogs = [
  {
    id: 1,
    name: "Flow Control in Javascript",
    tags: ["javascript", "asynchronous", "graphql"],
    date: "16 February, 2023",
    imgSrc:
      "https://media.graphassets.com/output=format:jpg/resize=height:800,fit:max/saIjdr9aSReTtmdVJcSi",
    link: "https://blog-next-app-phi.vercel.app/post/javascript-flow-control",
  },
  {
    id: 2,
    name: "The Most Popular React UI Component Libraries",
    tags: ["react", "styled-components", "material-UI", "React-Bootstrap"],
    date: "17 February, 2023",
    imgSrc:
      "https://media.graphassets.com/output=format:jpg/resize=height:800,fit:max/9WPzxKdhT7iqdGSG24xu",
    link: "https://blog-next-app-phi.vercel.app/post/react-ui-libraries",
  },
  {
    id: 3,
    name: "Top 9 Javascript Frameworks and Libraries to learn in 2023",
    tags: ["react", "frameworks", "libraries"],
    date: "13 March, 2023",
    imgSrc:
      "https://media.graphassets.com/output=format:jpg/resize=height:800,fit:max/MK7vRMxoRgiI8F6kXuk6",
    link: "https://blog-next-app-phi.vercel.app/post/js-frameworks",
  },
  {
    id: 4,
    name: "UX Writing and UX Design: How to Bring Them Together",
    tags: ["UI", "UX", "SEO Strategy"],
    date: "13 March, 2023",
    imgSrc:
      "https://media.graphassets.com/output=format:jpg/resize=height:800,fit:max/mffNQrd5TlC4phiGerSE",
    link: "https://blog-next-app-phi.vercel.app/post/user-experience",
  },
  {
    id: 5,
    name: "Characteristics of a good Test",
    tags: ["testing", "reactJS", "web-development"],
    date: "13 March, 2023",
    imgSrc:
      "https://media.graphassets.com/output=format:jpg/resize=height:800,fit:max/QCWddvIRFKIxdNa1LI1P",
    link: "https://blog-next-app-phi.vercel.app/post/testing",
  },
  {
    id: 6,
    name: "Deploying to GitHub Pages",
    tags: ["deploy", "github", "git"],
    date: "13 March, 2023",
    imgSrc:
      "https://media.graphassets.com/output=format:jpg/resize=height:800,fit:max/8FI1m9fSyBjCoXADwWGw",
    link: "https://blog-next-app-phi.vercel.app/post/deploy-github",
  },

  // {
  //   id: 7,
  //   name: "What is Redux ? Simply Explained!",
  //   tags: ["react", "redux", "javascript"],
  //   date: "16 May, 2021",
  //   imgSrc: "",
  //   link: "https://dev.to/codebucks/what-is-redux-simply-explained-2ch7",
  // },
  // {
  //   id: 8,
  //   name: "How to create Sidebar navigation menu in ReactJS with react router and framer-motion",
  //   tags: ["react", "framer-motion", "styled-components"],
  //   date: "26 May, 2021",
  //   imgSrc: "",
  //   link: "https://dev.to/codebucks/build-sidebar-navigation-menu-in-reactjs-with-react-router-and-framer-motion-for-cool-page-transition-effects-4bc3",
  // },
];
